...
@allure.tag("PEX", "2.5D", "MAGIC")
class Test(unittest.TestCase):
    @property
    def _sky130a_testdata_dir(self) -> str:
        return os.path.realpath(os.path.join(__file__, '..', '..', '..',
                                             'testdata', 'designs', 'sky130A'))

    def _gds(self, *path_components) -> str:
        return os.path.join(self._sky130a_testdata_dir, *path_components)
    ...
    def run_rcx25d_single_cell(self, *path_components) -> Tuple[CellExtractionResults, CSVPath, PNGPath]:
        gds_path = self._gds(*path_components)

        preview_png_path = tempfile.mktemp(prefix=f"layout_preview_", suffix=".png")
        self._save_layout_preview(gds_path, preview_png_path)
    ...
    def test_overlap_plates_100um_x_100um_li1_m1(self):
        self.assert_expected_matches_obtained(
...
C3;UPPER;VSUBS;205.52"""
    ...
    def test_overlap_plates_100um_x_100um_li1_m1_m2_m3(self):
...
C8;li1;met3;5.031"""
    ...
    def test_sidewall_100um_x_100um_distance_200nm_li1(self):
        # MAGIC GIVES (8.3 revision 485): (sorting changed to match order)
        # _______________________________ NOTE: with halo=8µm __________________________________
        # C2 C VSUBS 8.231f
        # C4 A VSUBS 8.231f
...
C5;A;B;7.5"""
    ...
    def test_near_body_shield_li1_m1(self):
        # MAGIC GIVES (8.3 revision 485): (sorting changed to match order)
        #_______________________________ NOTE: with halo=8µm __________________________________
